...
pub(crate) fn tour_panel(index: usize, theme: Theme) -> Container {
...
/// The paged walkthrough: the finger and the picker drive **one** page number, held by the
/// screen.
///
/// This is the whole point of the two-way binding: `on_page_changed` writes the page the
/// finger reached into the state, and `page` reads it back out. Neither side owns it, so
/// neither can drift from the other.
pub(crate) struct TourPage;
...
impl StatefulWidget for TourPage {
    type State = TourState;

    fn create_state(&self) -> TourState {
        TourState::default()
    }
}

impl State for TourState {
    type Widget = TourPage;
    ...
    fn build(&self, cx: &StateContext<Self>) -> Box<dyn Widget> {
        ...
        .flex(1.0)
        .page(page)
        .on_page_changed(cx.handler(|s, page: usize| s.page = page));
        ...
        let picker = Pagination::new(
            page + 1,
            TOUR_PAGES.len(),
            cx.handler(|s, p: usize| s.page = p - 1),
        );
        ...
        let skip = AnimatedPositioned::new(
            0.25,
            Curve::ease_out(),
            button("Skip", cx.callback(move |s| s.page = last))
        ...
        let screen = column![
            NavigationBar::new("Guided tour").on_back(on(move || {
                router.pop();
            })),
            panels,
            footer
...
mod tests {
    ...
    #[test]
    fn every_panel_has_a_glyph_a_title_and_a_line() {
        for (glyph, title, body) in TOUR_PAGES {
            assert!(!glyph.is_empty() && !title.is_empty() && !body.is_empty());
        }
    }
}
