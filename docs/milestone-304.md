...
- `scripts/seed-issues.sh` has not been run: `gh` is not installed on the maintainer's
  machine, and creating issues on a public tracker is the maintainer's to do. `--dry-run`
  works without the CLI or an account, deliberately — reading back what is about to be
  posted publicly should not itself need credentials.
