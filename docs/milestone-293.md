...
- `cargo apk build -p frus-demo --release` — 11,212,704 bytes against 11,209,608
  before the split, and `llvm-nm` finds `android_main` and `ANativeActivity_onCreate`
  in it. Worth checking rather than assuming: the entry point is generated by
  `frus_shell::main!`, which is the one line the split lost and had to have put back
  by hand — a crate that compiles and tests green with no entry symbol would have
  installed and done nothing.
