...
- 🟢 **`Scaffold` and `body` — reviewed, milestone 288.** `extend_body`, `extend_body_behind_app_bar`, `resize_to_avoid_bottom_inset`, `window_insets`, a leading `drawer` beside the trailing one, and `persistent_footer`. Left: **where the FAB sits** is still one corner, not a location (`floatingActionButtonLocation`), and there is no docked or notched variant; and an extended body is not *told* what it is running under, so its own content must be padded by hand.
