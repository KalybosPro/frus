...
- 🟡 **No bottom app bar, and so nowhere for a docked FAB to sit** (milestone 290). `FabLocation`'s docked placements are correct, but the only bottom bar frus has is the navigation one, whose destinations a docked button lands on. What is missing is the reference's `BottomAppBar`: a plain bar of actions, cut with a notch that receives the button.
