...
- 🟡 **Widen golden coverage — mostly done, milestone 296.** 58 of the 86 widget modules had no pixel test at all; 27 new goldens in `crates/frus-test/tests/widgets.rs` take that to 11. The eleven left all need a state a static render cannot supply — a swipe in flight (`Dismissible`), a drag, a route transition (`Navigator`, `Hero`, `Keyed`), a page offset (`PageView`), a pull (`Refresh`), a glow at an edge, and the two that read an ambient size (`LayoutBuilder`, `Responsive`). They want a harness that can drive the runtime forward a frame at a time, which is the unclaimed part.
