...
320 milestones in. The framework runs real, non-trivial applications on desktop and Android, and functional ones on the web. What exists is genuinely built, not stubbed: layout, text with IME, drag-and-drop with live reflow, data tables, charts, pickers, navigation with spring transitions, theming, i18n/RTL, accessibility, animation, async effects with typed JSON, and golden-image testing.
...
- 🔴 **`Scaffold` should not scroll the body.** Scrolling comes from a widget you choose — a list, a grid, a single scrolling child — and a scaffold's body is not one of them. Here every body is wrapped in a `Scroll` unconditionally, so an application cannot say *this screen does not scroll*, and a screen that fits still carries an offset, a physics and a set of edge effects it never asked for (milestone 316 stopped that wrapper from *acting*, which is not the same as it not being there). Undoing it touches every screen that exists and the keyboard-avoidance logic hanging off that viewport has to move with it, so the shape wants agreeing before code: which widgets scroll, what a body does when it overflows and nothing scrolls it, and how the migration reads.
