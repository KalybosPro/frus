...
318 milestones in. The framework runs real, non-trivial applications on desktop and Android, and functional ones on the web. What exists is genuinely built, not stubbed: layout, text with IME, drag-and-drop with live reflow, data tables, charts, pickers, navigation with spring transitions, theming, i18n/RTL, accessibility, animation, async effects with typed JSON, and golden-image testing.
...
- 🔴 **A widget that builds from the ambient theme.** The `caller ?? theme ?? framework` chain reaches painted properties (milestone 309) and layout (310), and stops at anything decided while a composition is being **assembled**. `AppBar::build()` never sees a theme, and `center_title` is not a property that gets painted — it decides which children exist and in what order, so no later hook can resolve it. `Themed` pushes a theme *down*; `LayoutBuilder` builds lazily but takes only a size and keeps no retained state; `Widget::children` returns a borrowed slice, so children must exist before the walk arrives. The fix is a widget that builds its subtree from the ambient theme during the themed walk, cached per frame — retained state survives, since the theme is stable frame to frame where a box is not. It unblocks `AppBar`, `Scaffold`, `NavBar` and the pickers at once, which is why the shape wants agreeing before code.
