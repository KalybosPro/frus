...
- 🔴 **The renderer does not composite in scene order** (found in milestone 291). It draws one pass per kind — rectangles, then images, then paths, then text — so *every* path is painted over *every* rectangle in the frame, wherever the two sit in the scene. It shows the moment a path is meant to sit **behind** something: a notched bottom app bar erases the fill of any filled button on it. Applies to every path primitive: `CustomPaint`, the charts, `ClipPath`, the overscroll glow. The fix is to interleave the passes by scene order — runs of same-kind primitives, one draw call per run — which means ranged draws in all four painters, and the text painter is the awkward one. Golden images and a device are the way to check it.
