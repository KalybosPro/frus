...
- 🔴 **A group-opacity layer from a covered page shows through the page above.** Found on the device in milestone 349: the home screen's translucent square is painted over the Kanban board, and thin slivers of a swipe background sit at the left edge of every screen. Layers are composited *after* the content batches — that is what makes one pass transform or fade a whole subtree — so a layer belonging to a page that something else is covering lands on top of the thing covering it. It needs the compositor to know which layers a covering page occludes, which is a question the batch planner does not currently ask.
- 🟡 **A virtualised list's rows hug their text instead of filling the list.** Also from the device, milestone 349: a `List` row with a background paints a chip around its label rather than a row across the list. The reference gives a list's children **tight** cross-axis constraints; ours leaves the cross axis free, so anything that does not set a width shrink-wraps.
