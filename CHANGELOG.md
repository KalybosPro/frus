...
> record — one per step, 547 so far, each documenting the objective, the alternatives
...
- **`frus-text` denies a missing doc comment** (J547, answers #14, continuing the
  sweep). Text measurement on top of `cosmic-text`, all in one 1,886-line `lib.rs`
  with feature-gated bundled fonts. `#![warn(...)]` found nothing, checked with
  `--all-features` and with the default build — straight to
  `#![deny(missing_docs)]`. Found while checking, not fixed (out of scope for this
  issue and pre-existing on `master`): `--no-default-features` fails on unrelated
  `dead_code` (`load_emoji_faces`, `FontDir`), code only reachable through the
  bundled-font features CI never turns off.
