...
- **`frus-core` denies a missing doc comment** (J548, answers #14, continuing the
  ...
  public API only through selective `pub use` at the crate root. Unlike the crates
  done so far, this one had a real, substantial list: `#![warn(...)]` found 92
  locations across eleven files, almost all struct/variant fields whose containing
  type already carried a doc comment but whose individual members didn't —
  `Color`'s channels, `Point`/`Insets`/`Size`'s coordinates, the eighteen named
  anchor constants on `Alignment`/`AlignmentDirectional`, the control points on
  `PathVerb`'s curve variants, several `Primitive` fields and
  `SceneBuilder::draw_rect` (which had no doc at all), `BorderSide` and the
  `side`/`radius` fields repeated across `ShapeBorder`'s variants, `TextRun`'s
  fields. Each is now documented with its unit or its role, not a restatement of
  its name. Promoted to `#![deny(missing_docs)]`; the crate's own 180 unit tests
  still pass.
- **`frus-fetch-example` denies a missing doc comment** (J543, answers #14,
  continuing the sweep). Same shape as `frus-hello`: `FetchDemo` and `Msg` are
  private, and the only public item is `run()`, generated by
  `frus::main!(FetchDemo::default())`. `#![warn(...)]` found nothing — every local
  item was already documented — straight to `#![deny(missing_docs)]`.
...
- **`frus-hello` denies a missing doc comment** (J542, answers #14, continuing the
  sweep). The framework's "Hello, world!" — a counter in one state struct, a pure
  `update` and a `view`. Neither `Counter` nor `Msg` is `pub`; the only public item is
  `run()`, generated by `frus::main!(Counter::default())`. `#![warn(...)]` found
  nothing: the macro-generated `run()` is not flagged, and every local item was
  already documented — straight to `#![deny(missing_docs)]`.
