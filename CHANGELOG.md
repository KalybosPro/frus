...
> record — one per step, 546 so far, each documenting the objective, the alternatives
...
- **`frus-demo` denies a missing doc comment** (J546, answers #14, continuing the
  sweep). Every module in `lib.rs` is private except one, `pub mod shots` (behind
  the `shots` feature), which already had a doc comment. A `pub` item inside a
  private module is not part of the effective public API, so despite 7,500 lines
  across twenty-odd files the crate's actual public surface is one function,
  `shots::write_previews`, already documented. `#![warn(...)]` found nothing,
  checked with and without `--all-features` — straight to `#![deny(missing_docs)]`.
