...
> record — one per step, 545 so far, each documenting the objective, the alternatives
...
- **`frus-test` denies a missing doc comment** (J545, answers #14, continuing the
  sweep). The framework's own test tooling. `#![warn(...)]` found one real gap:
  `Snapshot`'s three fields (`width`, `height`, `rgba`) had a doc comment on the
  struct but none of their own — now documented as pixels and as "four bytes per
  pixel, row-major from the top-left corner," not a restatement of their names.
  Everything else was already documented. Promoted to `#![deny(missing_docs)]`.
